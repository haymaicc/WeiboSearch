...
class WeiboSpiderSpider(scrapy.Spider):
    ...
    def start_requests(self):
        ...
        date_start = datetime.datetime.strptime("2018-12-29", '%Y-%m-%d')
        # 搜索的结束日期，自行修改
        date_end = datetime.datetime.strptime("2019-10-30", '%Y-%m-%d')
    ...
    def parse_tweet(self, response):
        ...
        for tweet_node in tweet_nodes:
            try:
                ...
                tweet_item['comment_num'] = comment_num

                # 抓取该微博的评论信息
                if comment_num > 0:
                    comment_url = tweet_node.xpath(
                        './/a[contains(text(),"评论[") and not(contains(text(),"原文"))]/@href').extract()[0]
                    print(comment_url)
                    yield Request(url=comment_url, meta={'item_id': id},
                                  callback=self.parse_comment, priority=2)
